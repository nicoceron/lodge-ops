  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-5 xl:grid-cols-2">
      ...
      <WorkspaceSection title="Deposits" subtitle="Due, paid, and waived deposit milestones.">
        <div className="space-y-2">{deposits.map((deposit) => <div key={deposit.id} className="flex items-center justify-between rounded-xl bg-[#faf8f2] px-4 py-3"><div><p className="text-xs font-bold">{formatMoney(deposit.amount_minor, deposit.currency)}</p><p className="mt-1 text-[10px] text-[var(--muted)]">{deposit.status} · due {displayDate(deposit.due_at)}</p></div>{deposit.status === "due" ? <button type="button" disabled={pending !== null} onClick={() => mutate(`deposit-${deposit.id}`, `deposits/${deposit.id}/waive`, { method: "POST", body: JSON.stringify({ reason: "Waived by staff from reservation workspace" }) })} className="text-[10px] font-bold text-[var(--red)]">Waive</button> : null}</div>)}</div>
        <form onSubmit={(event) => formMutation(event, "deposit-new", "deposits", (data) => ({ reservation_id: reservation.id, amount_minor: minorAmount(data.get("amount")), due_at: data.get("due_at") ? new Date(String(data.get("due_at"))).toISOString() : null }))} className="mt-3 grid grid-cols-[1fr_1fr_auto] gap-2"><input required name="amount" min="0.01" step="0.01" type="number" placeholder="Amount" className="field-input" /><input name="due_at" type="datetime-local" aria-label="Deposit due date" className="field-input" /><SubmitIcon pending={pending === "deposit-new"} label="Add deposit" /></form>
      </WorkspaceSection>

      <WorkspaceSection title="Payments" subtitle="Record cash, transfer, card, or other receipts.">
        <div className="space-y-2">{payments.map((payment) => <div key={payment.id} className="flex items-center justify-between rounded-xl bg-[#faf8f2] px-4 py-3"><div><p className="text-xs font-bold">{formatMoney(payment.amount_minor, payment.currency)} · {payment.method.replaceAll("_", " ")}</p><p className="mt-1 text-[10px] text-[var(--muted)]">{payment.status} · {displayDate(payment.processed_at)}</p></div>{payment.status === "pending" ? <button type="button" disabled={pending !== null} onClick={() => mutate(`payment-${payment.id}`, `payments/${payment.id}/reconcile`, { method: "POST", body: JSON.stringify({}) })} className="text-[10px] font-bold text-[var(--forest)]">Reconcile</button> : null}</div>)}</div>
        <form onSubmit={(event) => formMutation(event, "payment-new", "payments", (data) => ({ reservation_id: reservation.id, amount_minor: minorAmount(data.get("amount")), method: data.get("method"), provider_reference: data.get("reference") || null, provider: data.get("reference") ? "manual" : null, captured: data.get("captured") === "on" }))} className="mt-3 grid gap-2 sm:grid-cols-2"><input required name="amount" min="0.01" step="0.01" type="number" placeholder="Amount" className="field-input" /><select name="method" className="field-input"><option value="bank_transfer">Bank transfer</option><option value="cash">Cash</option><option value="card">Card</option><option value="other">Other</option></select><input name="reference" placeholder="Reference (optional)" className="field-input" /><label className="flex items-center gap-2 rounded-xl bg-[#faf8f2] px-3 text-xs font-semibold"><input name="captured" type="checkbox" defaultChecked />Already received</label><button disabled={pending !== null} className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-[var(--forest)] px-4 text-xs font-bold text-white sm:col-span-2">{pending === "payment-new" ? <LoaderCircle className="size-3.5 animate-spin" /> : <Plus className="size-3.5" />}Record payment</button></form>
      </WorkspaceSection>
