...
import { ArrowUpRight, BadgeDollarSign, Landmark, ReceiptText, WalletCards } from "lucide-react";
...
const metricIcons = [BadgeDollarSign, Landmark, ReceiptText, WalletCards];
const metricTones = {
  ...
  return (
    ...
    >
      ...
      <section className="surface-card mt-5 overflow-hidden rounded-2xl" aria-labelledby="channels-heading">
        ...
        {finance.channels.length ? <div className="scrollbar-slim overflow-x-auto"><table className="w-full min-w-[680px] text-left text-xs"><thead className="bg-[#faf8f2] text-[9px] tracking-[0.1em] text-[var(--muted)] uppercase"><tr><th className="px-5 py-3">Channel</th><th className="px-4 py-3">Bookings</th><th className="px-4 py-3">Revenue</th><th className="px-5 py-3 text-right">Collected</th></tr></thead><tbody className="divide-y divide-black/6">{finance.channels.map((item) => <tr key={item.channel}><td className="px-5 py-4 font-bold">{item.channel}</td><td className="px-4 py-4 font-mono">{item.bookings}</td><td className="px-4 py-4 font-mono">{formatMoney(item.revenueMinor, finance.currency)}</td><td className="px-5 py-4 text-right font-mono font-semibold">{item.collectionPercent}%</td></tr>)}</tbody></table></div> : <div className="px-5 py-10 text-center text-xs text-[var(--muted)]">No channel bookings in the current period.</div>}
      </section>
