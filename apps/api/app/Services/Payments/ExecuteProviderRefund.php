...
final class ExecuteProviderRefund
{
    public function __construct(
        ...
        private readonly RecoverProviderRefund $recover,
    ) {}
    ...
    public function handle(ReservationChange $request, ?int $actorId): ProviderRefund
    {
        ...
        if ($execution->state === ProviderRefundState::Succeeded
            && $execution->reservationChange->events()->where('type', 'refund_completed')->exists()) {
            return $execution;
        }
        ...
        try {
            ...
            return $this->recover->handle($execution->fresh(), $remote->id, $actorId);
