    ...
    public function after(): array
    {
        return [function (Validator $validator): void {
            foreach ($this->input('allocations', []) as $index => $allocation) {
                if (empty($allocation['resource_id']) && empty($allocation['service_occurrence_id'])) {
                    $validator->errors()->add("allocations.{$index}", 'An allocation must target a resource or service occurrence.');
                }
