...
Append `?fixture_state=paid_needs_review` (or any state in `fixtures/order-states.json`) to an order URL to render recovery/failure screens. Every state entry is a complete schema-valid order-status envelope whose actions are checked against the executable transition machine; evidence upload appears only after the manual-payment branch is selected, and rejected evidence exposes the retry transition. Distinct full error envelopes cover 403/404/409/410/422/503. `fixtures/manifest.json` lists every screen fixture. This proves UI/contract behavior only; it does not prove booking, inventory, payment or confirmation integration.
