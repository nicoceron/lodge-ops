...
`DirectBookingLaunchReadinessEvaluator` fails closed unless the property is active/enabled and every supported locale has effective, nonblank property copy/media/alt text plus exact category/program copy/media and terms, privacy, cancellation, no-show and marketing content. When bot verification is required, both the server-side Turnstile secret and a usable nonempty hostname allowlist are launch blockers. Every supported currency needs an active published rate plan/rule and at least one enabled payment capability. Hosted checkout additionally requires a connected payment integration, secret reference, nonblank provider account, exact charge currency and a provider supported by `PaymentGatewayFactory`; manual bank transfer requires an exact localized, effective instructions publication for every supported locale. The accessible fallback must be a query-free public HTTPS hostname (not credentials, localhost or an IP literal). Public media is only a query-free public HTTPS asset URL or an opaque `public-media://` reference; storage paths and signed private objects are forbidden.

All item/category/program, publication/item, capability/instruction and order/quote/reservation/payment-request associations are protected by tenant-and-property-inclusive database constraints. A category item can reference only category copy and a program item only program copy; the safe projection filters and deterministically orders exact-kind publications even in the presence of legacy contamination. Model predicates are defense in depth; wrong-kind, cross-tenant and same-tenant cross-property links are rejected by PostgreSQL triggers/composite foreign keys and equivalent SQLite enforcement triggers. The `060002` rollback preflights separate-expiry, recovery, instruction and PII-scrub facts before any DDL and refuses destructive rollback with those facts intact.
...
Append `?fixture_state=paid_needs_review` (or any state in `fixtures/order-states.json`) to an order URL to render recovery/failure screens, or `?fixture_error=quote_stale` (or any error in `fixtures/errors.json`) to return its exact HTTP status and full error envelope. Every state entry is a complete schema-valid order-status envelope whose actions are checked against the executable transition machine; evidence upload appears only after the manual-payment branch is selected, and rejected evidence exposes the retry transition. All 13 errors have distinct schema-valid OpenAPI and mock fixtures with exact status and retryability parity. `fixtures/manifest.json` lists every screen and error fixture. This proves UI/contract behavior only; it does not prove booking, inventory, payment or confirmation integration.
