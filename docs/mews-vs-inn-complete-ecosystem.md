...
Technology by feature is tracked in the [up-to-date technology audit](mews-vs-inn-technology-audit.md).

# Worker D audit — Mews ecosystem, security, API, Groups & Chains, and public website

Audit date: August 13, 2026. I cross-checked the live Mews Marketplace/site, official Mews help/docs, and the current Inn checkout.

Status:

- **Executable:** Inn has a real working implementation.
- **Partial:** Inn has an internal analogue, but not Mews-equivalent functionality.
...
Inn’ generic integration records are not counted as integrations unless there is a provider client and execution path.
...
| Mews Marketplace category | Current count | Representative capability | Inn status | Inn evidence |
|---|---:|---|---|---|
| Access Management | 3 | Smart locks, remote access, key systems | Absent | No provider client in [composer.json](../apps/api/composer.json:8) |
| Accounting | 38 | Accounting exports, ledgers, reconciliation | Adapter-only | Generic type only in [IntegrationConnection.php](../apps/api/app/Models/IntegrationConnection.php:7) |
| Analytics / Business Intelligence | 3 | External BI and analytics tools | Partial internally; no external adapter | Internal finance endpoint in [api.php](../apps/api/routes/api.php:49) |
...
| Customer management | 22 | CRM, guest data, reputation/customer tools | Partial internally; no external connector | Organizations/opportunities only in [api.php](../apps/api/routes/api.php:96) |
...
| Food & Beverage | 78 | Restaurant, bar, outlet, kitchen integrations | Partial retail only | Retail sale route in [api.php](../apps/api/routes/api.php:91) |
...
| Guest technology | 69 | Guest apps, concierge, Wi-Fi, room controls | Partial portal only | Guest portal routes in [api.php](../apps/api/routes/api.php:36) |
| HR & Staffing | Not rendered | Workforce, staffing, payroll tools | Partial internally | Roles/tasks exist; no external HR adapter |
| Integration management | 12 | Marketplace connection/subscription tools | Adapter-only | Configuration UI only in [IntegrationConnectionResource.php](../apps/api/app/Filament/Resources/IntegrationConnections/IntegrationConnectionResource.php:24) |
...
| Payments | 7 | Payment processors and payment services | Absent | Manual ledger only in [PaymentService.php](../apps/api/app/Services/PaymentService.php:23) |
...
| Integration class | Mews examples | Inn status | Inn evidence |
|---|---|---|---|
...
| Guest messaging | Aara, Akia, Duve | Partial email/portal only | Non-email channels throw an adapter error in [CommunicationDeliveryService.php](../apps/api/app/Services/CommunicationDeliveryService.php:30) |
...
| Marketplace administration | Connect, disable, disconnect, subscriptions | Adapter-only | Inn has configure/list only |
...
| Mews first-party product/add-on | Mews capability | Inn status | Inn evidence |
|---|---|---|---|
...
| Mews Multi-Property | Central portfolio management, bulk operations, unified property/guest data | Partial | Tenant/property model in [Tenant.php](../apps/api/app/Models/Tenant.php:26) |
...
| Text Messaging Package | Mews SMS guest communications | Absent | Non-email delivery rejected at [CommunicationDeliveryService.php](../apps/api/app/Services/CommunicationDeliveryService.php:30) |
...
| Mews POS/ePOS | Guest-centric restaurant POS, tableside ordering/payment, inventory upgrades | Partial retail only | Retail route only in [api.php](../apps/api/routes/api.php:91) |
| Mews Payments | Embedded processor payments | Absent | [PaymentService.php](../apps/api/app/Services/PaymentService.php:23) records manual payments |
...
| Connector API | PMS data/services integration API | Absent as compatible API | Inn API is internal only |
...
| Flexkeeping | Dynamic housekeeping schedules, staff forecast, maintenance, AI voice, contractors | Partial internal | Inn has housekeeping state/tasks, not Flexkeeping scope |
...
| Open API platform | APIs, webhooks, portfolio integration | Partial internal API only | [api.php](../apps/api/routes/api.php:45) |
...
| Groups & Chains capability | Inn status | Inn evidence |
|---|---|---|
...
| Shared roles across properties | Partial | Role exists in [MembershipRole.php](../apps/api/app/Enums/MembershipRole.php:5), but no central bulk role administration |
...
| Security/compliance capability | Inn status | Inn evidence |
|---|---|---|
| Application authentication | Executable | Laravel session guard in [auth.php](../apps/api/config/auth.php:40) |
| Sanctum API authentication | Executable | Sanctum middleware in [api.php](../apps/api/routes/api.php:45) |
| Tenant isolation | Executable | [ResolveTenant.php](../apps/api/app/Http/Middleware/ResolveTenant.php:18) |
| Property-scoped membership | Executable | [Membership.php](../apps/api/app/Models/Membership.php:16) |
| Role-based permissions | Executable | [TenantPolicy.php](../apps/api/app/Policies/TenantPolicy.php:9) |
| Finance/operations role separation | Executable | [MembershipRole.php](../apps/api/app/Enums/MembershipRole.php:23) |
| MFA/TOTP | Executable | Filament MFA traits in [User.php](../apps/api/app/Models/User.php:29) |
| MFA recovery | Executable | Filament recovery trait in [User.php](../apps/api/app/Models/User.php:29) |
| Email verification | Executable | `MustVerifyEmail` in [User.php](../apps/api/app/Models/User.php:29) |
| Password recovery | Executable | Password broker in [auth.php](../apps/api/config/auth.php:95) |
| Audit records | Executable | [Audit.php](../apps/api/app/Models/Audit.php:8) |
| Immutable audit records | Executable | Update/delete protection in [Audit.php](../apps/api/app/Models/Audit.php:20) |
| Idempotent command handling | Executable | `idempotent` middleware in [bootstrap/app.php](../apps/api/bootstrap/app.php:23) |
| Tenant context restoration for jobs | Executable | [PublishOutboxMessage.php](../apps/api/app/Jobs/PublishOutboxMessage.php:39) |
| Encrypted staff sessions | Executable by Laravel/Filament | Documented in [security/page.tsx](../apps/web/src/app/security/page.tsx:10) |
| CSRF protection | Executable by Laravel/Filament | Documented in [security/page.tsx](../apps/web/src/app/security/page.tsx:10) |
| Hashed guest portal tokens | Executable | [GuestPortalAccessToken.php](../apps/api/app/Models/GuestPortalAccessToken.php:8) |
| Security headers | Executable in public Next app | [next.config.ts](../apps/web/next.config.ts:11) |
...
| Uptime SLA | Absent | No Inn SLA/status contract |
...
| Mews API/Marketplace capability | Inn status | Inn evidence |
|---|---|---|
| Public developer API | Partial internal API only | Internal `/api/v1` in [api.php](../apps/api/routes/api.php:45) |
| Open API documentation portal | Absent | No public Inn developer portal |
...
| Enable/disable integration state | Partial local state only | `status` is stored in [IntegrationConnectionService.php](../apps/api/app/Services/IntegrationConnectionService.php:27), but does not control data transfer |
| Disconnect/delete provider config | Partial local configuration only | Resource deletion is disabled in [IntegrationConnectionResource.php](../apps/api/app/Filament/Resources/IntegrationConnections/IntegrationConnectionResource.php:40) |
...
| External email delivery | Executable | Laravel mail transport in [CommunicationDeliveryService.php](../apps/api/app/Services/CommunicationDeliveryService.php:80) |
| Private iCalendar feed | Executable, one-way | [CalendarFeedService.php](../apps/api/app/Services/CalendarFeedService.php:15) |
| Internal outbox | Executable, internal only | [OutboxBatchPublisher.php](../apps/api/app/Services/Automation/OutboxBatchPublisher.php:21) |
| External webhook delivery | Absent | Outbox dispatches internal automation jobs only |
| External payment execution | Absent | Manual ledger behavior in [PaymentService.php](../apps/api/app/Services/PaymentService.php:23) |

The exact Inn integration boundary is six configuration types:
...
| Website capability | Mews live observation | Inn status | Inn evidence |
|---|---|---|---|
| Frontend framework | Next.js/App Router/RSC markers | Partial | Next.js dependency in [package.json](../apps/web/package.json:13) |
| React runtime | React frontend | Executable | React dependency in [package.json](../apps/web/package.json:13) |
...
| Seven locale sites | `de`, `en`, `en-gb`, `es`, `fr`, `it`, `nl` | Absent | Root layout is `lang="en"` in [layout.tsx](../apps/web/src/app/layout.tsx:25) |
| Localized sitemap index | Seven locale sitemap files | Absent | No `sitemap.ts` or sitemap files in [apps/web/src/app](../apps/web/src/app) |
| English sitemap size | 2,734 URLs in current snapshot | Absent | Inn has no public content sitemap |
...
| Product catalog | Large platform catalog with product pages | Partial | Inn markets core workflows in [page.tsx](../apps/web/src/app/page.tsx:5) |
...
| GTM | Google Tag Manager script observed | Absent | No GTM in Inn web source |
...
| Motion system | Motion-named chunks and `LazyMotion` markers | Absent | No Motion dependency in [package.json](../apps/web/package.json:13) |
...
| Font preloading/self-hosted font pipeline | Multiple preloaded WOFF2 assets | Partial | Inn imports Google Fonts in [globals.css](../apps/web/src/app/globals.css:1) |
| Public contact/demo funnel | Contact/demo navigation present; exact form execution not inferred from HTML alone | Partial marketing links | Inn only links to staff application |
| Public auth boundary | Mews login link and operational platform separation | Partial | Inn links to Laravel `/manage` from [layout.tsx](../apps/web/src/app/layout.tsx:39) |
| Security headers | Mews headers observed through hosting; exact policy not fully inferred | Executable locally | Inn defines headers in [next.config.ts](../apps/web/next.config.ts:11) |
| Public product navigation | Mews platform, solutions, resources, company, legal, careers | Partial | Inn has home/features/pricing/security in [layout.tsx](../apps/web/src/app/layout.tsx:33) |
...
| Current Mews capability | Availability/status note | Inn status | Inn evidence | Mismatch |
|---|---|---|---|---|
| Mews OS unified operating-system layer | Current platform positioning; umbrella rather than a separate deployable module | PARTIAL | [api.php](../apps/api/routes/api.php:45) | Inn has internal modules but no unified PMS/POS/RMS/payments/distribution operating system. |
| Native Mews Channel Manager powered by SiteMinder | Announced May 2026; rollout/availability may be property- or market-gated | ABSENT | [IntegrationConnection.php](../apps/api/app/Models/IntegrationConnection.php:5) | No channel-manager execution, OTA mapping, or SiteMinder adapter exists. |
| Native distribution through 400+ OTAs | Public Mews/SiteMinder product claim | ABSENT | [api.php](../apps/api/routes/api.php:45) | No OTA/channel publication or reservation ingestion exists. |
| One contract, one bill, and one support line for native distribution | Public Mews/SiteMinder product claim | ABSENT | [IntegrationConnectionService.php](../apps/api/app/Services/IntegrationConnectionService.php:8) | Inn has no provider contracting, billing, or support integration layer. |
| ePOS as a separately marketed product surface | Current Mews catalog lists ePOS separately from POS | ABSENT | [api.php](../apps/api/routes/api.php:88) | Inn has retail sales, not restaurant ePOS, tableside ordering, or embedded F&B payments. |

## Public GitHub stack-source cross-check

The attached GitHub research was checked, but public code is kept separate from product parity claims. The full link inventory and exact API deltas are in the [GitHub/OpenAPI re-audit](mews-vs-inn-github-openapi-audit.md).
