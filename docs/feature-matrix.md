...
| Reservations | Manual entry, guests/companions, itinerary, stays, activities, append-only note timeline, amend/cancel, actual arrival/departure facts and reasoned cancellation/no-show closure | Filament + API | Implemented — `GenericOperationalFlowsTest`, `ReservationApiTest`, `ReservationStatusTest`, `FilamentResourcesTest` |
...
| Integrations | Email/calendar/accounting/payment/e-sign/webhook adapters plus private per-resource iCalendar channel feeds | Filament + API | Implemented adapter boundary — `GenericOperationalFlowsTest`, `ExtendedOperationsTest`, outbox suites; provider activation requires tenant-owned credentials |
