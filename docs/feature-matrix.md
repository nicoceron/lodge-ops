...
| Payments | Configurable fixed/percentage deposits, balance offsets, manual bank transfer evidence, secure staff review, exact-once payment/folio/deposit reconciliation, and append-only internal partial/full refund lifecycle | Filament + Laravel guest portal | Manual record/reconcile and cancellation/refund are browser-proven — `UAT-4.5`, `PaymentEvidenceReviewTest`, `ReservationChangesTest`; guest evidence approval UAT, provider checkout/webhooks/refunds, disputes, receipts, and settlement remain |
