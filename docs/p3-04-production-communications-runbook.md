...
3. As an authorized Inn administrator, create one enabled provider connection for the property. Enter the verified sender domain, exact from/reply-to identities, provider account label and sender-domain allowlist. Copy the opaque endpoint key once into the Resend webhook URL.
...
Run `php artisan horizon` under the process manager. Required queues are `critical`, `provider-events`, `notifications`, `automations`, `documents`, `reports`, `integrations`, and `default`; `config/horizon.php` isolates latency-sensitive event/send work from document/report work. Redis `retry_after` is 240 seconds and every worker timeout is lower. Agent 13 must replace the ordinary Compose `queue:work` worker with this Horizon process when integrating production topology.

On deploy, run migrations, start the new release, and run `php artisan horizon:terminate`; the process manager starts workers on the new code. `horizon:snapshot` runs every five minutes. The Horizon dashboard is authorized only for a user with an active Administrator membership.
...
The minute scheduler has a named ten-minute lock and `onOneServer()`. Occurrences persist property-local input, timezone, UTC instant, rule/policy versions and claim identity. PostgreSQL claims use `FOR UPDATE SKIP LOCKED`; jobs are enqueued after commit. Amendments supersede pending/claimed old revisions, cancellations/no-shows suppress them, and dispatched history is never rewritten.
...
Alert when the `reservation-milestones` row in `scheduler_heartbeats` is absent or `last_seen_at` is older than two scheduler intervals. Also alert on rising `reconciliation_required`, failed `provider-events`/`notifications`, queue waits over Horizon thresholds, or provider complaint/hard-bounce spikes.
...
Only authorized, unexpired, checksum-verified app-generated PDFs may be attached. Uploaded/evidence artifacts remain blocked until Agent 12's clean-scan state is integrated. Logs and normalized webhook payloads contain hashes and safe codes, not recipients, bodies, API keys, webhook secrets or raw provider payloads.
