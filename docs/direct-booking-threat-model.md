...
| Bot hold denial | Inventory is exhausted by fake holds | Server-side Turnstile on begin/hold, single-use action/hostname checks, per-session hold/hour limit, property/IP limits, commit-time lock, short bounded expiry | Turnstile fake with stray HTTP blocked; hold schedule |
| Token theft/replay | IDOR and guest-data exposure | 64-character opaque bearer, hash-only persistence, property binding, generic failure, rotation/revocation, no-store, no URL/query token, rate limits | Token issue/rotate/revoke/expiry/property tests |
| IDOR / tenant crossover | Other property or tenant accessed | Property slug + token binding, composite tenant foreign keys, global tenant scope after resolution, generic 404, no internal UUIDs | Composite-FK and cross-property token tests |
...
| Late payment / inventory loss | Paid guest lacks capacity | Expired or failed commit becomes `paid_needs_review`; Finance recovers valid inventory or provider-refunds; never auto-confirms | Explicit late-payment transition test |
...
| Stale/replayed mutation | Wrong lifecycle or duplicate side effect | Required idempotency key, canonical checksum, exact replay, different-body conflict, expected state version, DB unique retry identity | Replay/different-body/version and PostgreSQL race tests |
| Undefined policy/copy | Guest accepts invented text | Only immutable published source may render; missing locale/version is generic 404 and readiness blocker | Publication constraints/readiness evaluator |
...
Turnstile validation is server-side. Tokens are treated as short-lived, single-use credentials and are submitted with the expected action, hostname allowlist, remote IP when available and the stable command idempotency key. Network/error/missing-secret results fail closed. Tests fake the exact endpoint and prevent stray HTTP. See [Cloudflare server-side validation](https://developers.cloudflare.com/turnstile/get-started/server-side-validation/).
...
Never place the following in source, logs, exceptions, queues, outbox payloads, analytics, exports, screenshots or evidence: raw direct-booking/guest/payment tokens, Turnstile token, voucher code, email, phone, raw IP, bank evidence/content, card PAN/CVV/expiry/track/PIN, provider access/webhook secret, provider payload or signed private-media URL. Safe events contain only opaque order reference, event name, state/version, UTC time, correlation ID and an allowlisted reason code.
...
Agent 07 must connect these controls to real availability/quote/commit/payment/evidence services, implement exact HTTP response replay in the public tenant context, enforce one active competing payment request, emit safe events after commit and add real PostgreSQL provider/inventory races. Agent 08 must use only the public DTOs and exercise every pending/failure/recovery state. Agent 09 must prove the complete browser journey, edge/WAF behavior, real hosted payment/manual review, late payment, cleanup and accessibility. Until then, the foundation route is deliberately unavailable.
