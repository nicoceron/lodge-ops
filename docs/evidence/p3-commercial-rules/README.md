...
| Reviewer regression matrix | PASS | 15 focused SQLite tests, 232 assertions, plus 5 second-review PostgreSQL tests / 170 assertions: governing-rule priority/no-fallback and non-UTC/DST dates; atomic publication; per-promotion/voucher usage; currency-filtered tax; authoritative fiscal-source reconciliation; all-role explanation/configuration policies; pre-DDL rollback guard; stable staff session identity; immutable voucher-redemption facts with valid lifecycle transitions; refund lifecycle/idempotency; complete historical projection. |
