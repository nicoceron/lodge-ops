...
The separate Guide journey creates, edits, visibly verifies, and deletes an availability block through the authenticated Guide browser session for the Guide-linked resource. It also attempts to mutate a seeded block owned by another Guide and proves the policy denial, then verifies Guest, Reservation, Payment, Property, and Finance surfaces remain inaccessible. The Guide calendar contains only the Guide's assignments and minimized operational need. The redacted availability screenshot and Manager/Guide traces are Playwright attachments uploaded by CI as the `authenticated-client-uat-*` artifact; no unredacted guest or payment content is retained.
