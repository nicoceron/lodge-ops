...
- Connection identity includes tenant, property scope, provider, product, external account, and environment. Point/QR capabilities and exact charge currency must be enabled; terminal/POS mappings cannot be silently reassigned across properties/connections.
...
- Create/cancel/refund use stable operation-specific idempotency identities and canonical checksums. Busy/already-queued and ambiguous outcomes recover by authoritative lookup.
- Money applies only after account/type/reference/device-or-POS/amount/currency/order/transaction validation. Duplicate/reordered/late events and concurrent approval retain one Payment, folio effect, receipt request, and settlement revision.
...
- QR buyer rejection is not invented as an order failure. Active QR is encrypted at rest and erased on terminal state; the staff response renders a real QR, not the raw payload.
...
- The final SQLite suite passed `473` tests (`437` passed, `36` expected PostgreSQL-only skips) with `3,628` assertions. The final PostgreSQL suite passed all `473` tests with `3,803` assertions and one explicit non-PostgreSQL/environment skip.
...
- The isolated `inn_agent02` Compose stack ran with API `8202`, web `3202`, PostgreSQL `5532`, and Redis `6472`; API/web health checks, Filament assets, public site, worker, scheduler, PostgreSQL, Redis, and Mailpit passed `make doctor`.
- Final focused Chromium passed `2/2`: Point traversed signed HTTP delivery and the normal `provider-events` worker into one approved payment, receipt, and settlement; QR rendered a real SVG at `390`, `768`, and `1440` pixel widths and disappeared after authoritative approval. The public-browser suite passed `4/4`, and the tender browser regression changed while merging current `main` passed its focused journey `1/1`.
